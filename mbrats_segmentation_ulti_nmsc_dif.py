...
def run(args):
    ...
    def prepare_batch(batch, slice_conditional=False):
        ...
        if not slice_conditional:
            hi_1 = si_1 = hi_2 = si_2 = None
        return h_1, s_1, m_1, hi_1, si_1, h_2, s_2, m_2, hi_2, si_2
    ...
    def training_function(engine, batch):
        for model in experiment_state.model.values():
            model.train()
        SB, SA, SA_M, I_SB, I_SA, TB, TA, TA_M, I_TB, I_TA = prepare_batch(batch, args.slice_conditional)
    ...
    def output_transform(output):
        ...
        for k, v in output.items():
            if k.startswith('x_') and v is not None and v.dim()==4:
                k_new = k.replace('x_','')
                v_new = v.cpu().numpy()
                if k_new.startswith('SM') or k_new.startswith('TM') or k_new.startswith('SATM') or k_new.startswith('TAM') or k_new.startswith('SAM'):
                    if v_new.shape[1]==1:
