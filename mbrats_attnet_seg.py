...
def get_parser():
    ...
    g_exp.add_argument('--labeled_fraction_1', type=float, default=1)
    g_exp.add_argument('--labeled_fraction_2', type=float, default=0)
...
def run(args):
    ...
    from utils.data.multimodal_brats import (prepare_attnet,
                                  preprocessor_attnet)
    ...
    # Prepare data.
    data = prepare_attnet(path=os.path.join(args.data),
                              modalities = [args.source_modality, args.target_modality],
                              masked_fraction_1=1.-args.labeled_fraction_1,
                              masked_fraction_2=1.-args.labeled_fraction_2,
                              rng=np.random.RandomState(args.data_seed))

    get_data_list = lambda key : [data[key]['s_1'],
                                  data[key]['m_1'],
                                  data[key]['s_2'],
                                  data[key]['translated_s_2'],
                                  data[key]['m_2']]
    loader = {
        'train': data_flow_sampler(get_data_list('train'),
                                   sample_random=True,
                                   batch_size=args.batch_size_train,
                                   preprocessor=preprocessor_attnet(data_augmentation_kwargs=da_kwargs),
        ...
        'valid': data_flow_sampler(get_data_list('valid'),
                                   ...
                                   preprocessor=preprocessor_attnet(),
                                   nb_io_workers=args.nb_io_workers,
                                   nb_proc_workers=args.nb_proc_workers,
                                   rng=np.random.RandomState(args.init_seed)),
    ...
    def prepare_batch(batch):
        s_1, m_1, s_2, translated_s_2, m_2 = batch
        # Prepare for pytorch.
        s_1 = Variable(torch.from_numpy(np.array(s_1))).cuda()
        s_2 = Variable(torch.from_numpy(np.array(s_2)))
        translated_s_2 = Variable(torch.from_numpy(np.array(translated_s_2))).cuda()
        return s_1, m_1, s_2, translated_s_2, m_2
    ...
    def training_function(engine, batch):
        for model in experiment_state.model.values():
            model.train()
        S, SM, T, TS, TSM = prepare_batch(batch)

        outputs = experiment_state.model['G'](x_S=S, x_T=T, x_TS=TS, mask_S=SM, mask_T=TSM, optimizer=experiment_state.optimizer)
    ...
    def validation_function(engine, batch):
        ...
        S, SM, T, TS, TSM = prepare_batch(batch)
        with torch.no_grad():
            outputs = experiment_state.model['G'](x_S=S, x_T=T, x_TS=TS, mask_S=SM, mask_T=TSM, rng=engine.rng)
        outputs = detach(outputs)
        return outputs
    ...
    def dice_transform_all_T(x):
        return (x['x_TSM_main'], x['x_TM'])       
    def dice_transform_all_S(x):
        return (x['x_SM_main'], x['x_SM'])          
    ...
    for key in engines:
        ...
        metrics[key]['D_main']    = batchwise_loss_accumulator(
                        output_transform=lambda x: x['l_D_main'])
        metrics[key]['D_aux']    = batchwise_loss_accumulator(
                        output_transform=lambda x: x['l_D_aux'])
