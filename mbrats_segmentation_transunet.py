...
def get_parser():
    ...
    g_exp.add_argument('--data', type=str, default='./Data/BraTS/2020')
...
def run(args):
    ...
    from utils.data.multimodal_brats import preprocessor_transunet
    from utils.data.multimodal_brats import prepare_base
    ...
    from model.Transunet_segmentation import segmentation_model
    ...
    data = prepare_base(path=os.path.join(args.data, "data.h5"),
    ...
    loader = {
        'train': data_flow_sampler(get_data_list('train'),
                                   ...
                                   preprocessor=preprocessor_transunet(
        ...
        'valid': data_flow_sampler(get_data_list('valid'),
                                   ...
                                   nb_proc_workers=args.nb_proc_workers,
                                   rng=np.random.RandomState(args.init_seed)),
        'valid_1': data_flow_sampler(get_data_list('valid_1'),
                                   sample_random=True,
                                   batch_size=args.batch_size_valid,
                                   preprocessor=preprocessor_transunet(),
                                   nb_io_workers=args.nb_io_workers,
                                   nb_proc_workers=args.nb_proc_workers,
                                   rng=np.random.RandomState(args.init_seed)),
        'valid_2': data_flow_sampler(get_data_list('valid_2'),
                                   sample_random=True,
                                   batch_size=args.batch_size_valid,
                                   preprocessor=preprocessor_transunet(),
                                   nb_io_workers=args.nb_io_workers,
        ...
        'test':  data_flow_sampler(get_data_list('test'),
                                   ...
                                   nb_proc_workers=args.nb_proc_workers,
                                   rng=np.random.RandomState(args.init_seed)),        
        'test_1':  data_flow_sampler(get_data_list('test_1'),
                                   sample_random=True,
                                   batch_size=args.batch_size_valid,
                                   preprocessor=preprocessor_transunet(),
                                   nb_io_workers=args.nb_io_workers,
                                   nb_proc_workers=args.nb_proc_workers,
                                   rng=np.random.RandomState(args.init_seed)),
        'test_2':  data_flow_sampler(get_data_list('test_2'),
                                   sample_random=True,
                                   batch_size=args.batch_size_valid,
                                   preprocessor=preprocessor_transunet(),
                                   nb_io_workers=args.nb_io_workers,
    ...
    def training_function(engine, batch):
        ...
        outputs = experiment_state.model['G'](A, M,
    ...
    def validation_function(engine, batch):
        ...
        B, A, M, I_A, I_B = prepare_batch(batch, args.slice_conditional)
        with torch.no_grad():
            outputs = experiment_state.model['G'](A, M, rng=engine.rng)
        outputs = detach(outputs)
        return outputs
    ...
    engines['valid_1'] = experiment_state.setup_engine(
                                            validation_function,
                                            prefix='valid_1',
                                            epoch_length=len(loader['valid_1']))
    engines['valid_2'] = experiment_state.setup_engine(
                                            validation_function,
                                            prefix='valid_2',
                                            epoch_length=len(loader['valid_2']))
    engines['test'] = experiment_state.setup_engine(
                                            validation_function,
                                            prefix='test',
                                            epoch_length=len(loader['test']))
    engines['test_1'] = experiment_state.setup_engine(
                                            validation_function,
                                            prefix='test_1',
                                            epoch_length=len(loader['test_1']))
    engines['test_2'] = experiment_state.setup_engine(
                                            validation_function,
                                            prefix='test_2',
                                            epoch_length=len(loader['test_2']))
    for key in ['valid', 'test', 'valid_1', 'test_1', 'valid_2', 'test_2']:
    ...
    def dice_transform_all(x):
        ...
        return (x['x_AM'][:,:,8:248,68:188], x['x_M'][:,:,8:248,68:188])                # 1 out; get whole lesion.
    ...
    engines['train'].add_event_handler(Events.EPOCH_COMPLETED,
                             lambda _: engines['valid_1'].run(loader['valid_1']))
    engines['train'].add_event_handler(Events.EPOCH_COMPLETED,
                             lambda _: engines['valid_2'].run(loader['valid_2']))
    ...
    for key in ['train', 'valid', 'valid_1', 'valid_2']:
        tracker.attach(
            engine=engines[key],
            prefix=key,
                ...
                if k_new=='M' or k_new.startswith('AM'):
                    if v_new.shape[1]==1:
                        ...
                        v_new = sum([(v_new==i+1)*c
                                     for i, c in enumerate(target_class)])
                    if output['x_AM'] is not None and output['x_AM'].shape[1]!=1:
                        # HACK (min and max values for correct vis)
                        v_new[:,0,0]  = max(target_class)
                        v_new[:,0,-1] = 0
                else:
                    v_new = v_new[:,0]         # 4 sequences per img.
    ...
    for modality in modalities:
        save_image = image_logger(
            ...
            fontsize=40)
        if i==0:
            save_image.attach(engines['valid_1'])
            i+=1
        else:
            save_image.attach(engines['valid_2'])
    ...
    engines['test_1'].run(loader['test_1'])
    engines['test_2'].run(loader['test_2'])
