# MGenSeg with attention gates as skip connections
...
def build_model(lambda_disc=1, 
    ...
    decoder_common_kwargs = {
        ...
        'nonlinearity'        : lambda : nn.ReLU(inplace=True),
        'long_skip_merge_mode': 'attention',
        'ndim'                : 2}
...
class decoder(nn.Module):
    def __init__(self, input_shape, output_shape, num_conv_blocks, block_type,
        ...
        valid_modes = [None, 'skinny_cat', 'cat', 'pool', 'attention']
        ...
        for n in range(self.num_conv_blocks):
            ...
            if   self.long_skip_merge_mode=='skinny_cat':
                ...
                last_channels *= 2
            elif self.long_skip_merge_mode=='attention':
                cat = Attention_block(F_g=self.num_channels_list[n], F_l=self.num_channels_list[n], F_int=int(self.num_channels_list[n]/2))
                self.cats.append(cat)                
                last_channels *= 2
            else:
                pass
        ...
        for n, block in enumerate(self.blocks):
            if (self.long_skip_merge_mode=='pool' and skip_info is not None
                ...
                if   self.long_skip_merge_mode=='skinny_cat':
                    ...
                    att_gate = self.cats[n]
                    att = att_gate(g=out,x=skip)
