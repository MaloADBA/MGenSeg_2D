...
from model.gen_segmentation_ulti_nmsc_dif import segmentation_model
...
def build_model(lambda_disc=1, 
    ...
    if lambda_enforce_sum is not None:
        lambda_sum_genseg = ( lambda_disc
                      +lambda_x_id
                      +lambda_z_id
                      +lambda_seg)
        
        lambda_scale_genseg = lambda_enforce_sum/lambda_sum_genseg
        
        lambda_sum = ( lambda_genseg
                      +lambda_mod_disc
                      +lambda_mod_x_id
                      +lambda_mod_z_id
                      +lambda_mod_cyc)
    ...
    encoder_kwargs = {
        ...
        'ndim'                : 2}
    encoder_instance_source = encoder(**encoder_kwargs)
    encoder_instance_target = encoder(**encoder_kwargs)
    enc_out_shape = encoder_instance_source.output_shape
    ...
    submodel = {
        ...
        'decoder_common_source'    : decoder(**decoder_common_kwargs),
        'decoder_common_target'    : decoder(**decoder_common_kwargs),
        'decoder_residual_source'  : decoder(**decoder_residual_kwargs),
        'decoder_residual_target'  : decoder(**decoder_residual_kwargs),
        'decoder_source'    : decoder(**decoder_trans_kwargs),
        'decoder_target'  : decoder(**decoder_trans_kwargs),        
        'segmenter'         : None,
        'disc_SA'            : munit_discriminator(**discriminator_kwargs),
        'disc_SB'            : munit_discriminator(**discriminator_kwargs),
    ...
    #remove_spectral_norm(submodel['decoder_residual'].out_conv[2].conv.op)
    #remove_spectral_norm(submodel['decoder_residual'].out_conv[3].conv.op)
    remove_spectral_norm(submodel['decoder_residual_source'].classifier.op)
    remove_spectral_norm(submodel['decoder_residual_target'].classifier.op)
    ...
    model = segmentation_model(**submodel,
                               ...
                               lambda_disc=lambda_disc*lambda_scale_genseg*lambda_genseg*lambda_scale,
                               lambda_x_id=lambda_x_id*lambda_scale_genseg*lambda_genseg*lambda_scale,
                               lambda_z_id=lambda_z_id*lambda_scale_genseg*lambda_genseg*lambda_scale,
                               lambda_seg=lambda_seg*lambda_scale_genseg*lambda_genseg*lambda_scale,
                               lambda_mod_disc=lambda_mod_disc*lambda_scale,
                               lambda_mod_x_id=lambda_mod_x_id*lambda_scale,
                               lambda_mod_z_id=lambda_mod_z_id*lambda_scale,
                               lambda_mod_cyc=lambda_mod_cyc*lambda_scale)
    
    return OrderedDict((
        ('G', model),
        ('D', nn.ModuleList([model.separate_networks['disc_SA'],
                             model.separate_networks['disc_SB'],
                             model.separate_networks['disc_TA'],
                             model.separate_networks['disc_TB'],
                             model.separate_networks['disc_S'],
...
class switching_normalization(nn.Module):
    def __init__(self, normalization, *args, **kwargs):
        ...
        self.norm0 =normalization(*args, **kwargs) #towards residual
        self.norm1 = normalization(*args, **kwargs) #towards segmentation
        self.mode = 0
